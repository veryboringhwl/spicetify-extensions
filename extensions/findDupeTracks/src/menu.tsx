...
export function PlaylistDuplicateFinder({
  ...
  const handleConfirmDeleteAll = () => {
    ...
    void (async () => {
      const itemsToRemove = duplicatesToDelete.map((t) => ({ uid: t.uid }));
      const uidsToRemove = new Set(duplicatesToDelete.map((t) => t.uid));
      await Spicetify.Platform.PlaylistAPI.remove(currentPlaylist.uri, itemsToRemove);

      setTracks((prev) => prev.filter((t) => !uidsToRemove.has(t.uid)));
    })();
